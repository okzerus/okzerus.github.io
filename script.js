// script.js - full app (chapters, blur, tooltip images, image viewer, color picker, edge button, glow, glitch)
// Full replacement: overwrite existing script.js with this file.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // edge button and position state (declared early)
  let edgeBtn = null;
  ...
  function applyColorHex(hex){
    const root = document.documentElement; const {r,g,b} = hexToRgb(hex);
  ...
  function addDrag(element, handlers){
    ...
    element.addEventListener('pointerdown', (ev)=>{ element.setPointerCapture && element.setPointerCapture(ev.pointerId); dragging=true; pointerId=ev.pointerId; handlers.start && handlers.start(ev); ev.preventDefault(); });
    window.addEventListener('pointermove', (ev)=>{ if(!dragging || (pointerId!==null && ev.pointerId!==pointerId)) return; handlers.move && handlers.move(ev); ev.preventDefault(); }, {passive:false});
    window.addEventListener('pointerup', (ev)=>{ if(!dragging || (pointerId!==null && ev.pointerId!==pointerId)) return; dragging=false; pointerId=null; handlers.end && handlers.end(ev); ev.preventDefault(); });
  ...
  /* wiring for color picker */
  function handleHuePointer(e){ if(!hueSlider) return; const rect=hueSlider.getBoundingClientRect(); const y=Math.min(Math.max(0,(e.clientY||0)-rect.top), rect.height); const ratio=1-(y/rect.height); hsv.h = ratio*360; updatePickerUI(); applyHsvState(); persistHex(rgbToHex(...Object.values(hsvToRgb(hsv.h,hsv.s,hsv.v)))); }
  function handleAreaPointer(e){ if(!colorArea) return; const rect=colorArea.getBoundingClientRect(); const x=Math.min(Math.max(0,(e.clientX||0)-rect.left), rect.width); const y=Math.min(Math.max(0,(e.clientY||0)-rect.top), rect.height); hsv.s = (x/rect.width); hsv.v = 1 - (y/rect.height); updatePickerUI(); applyHsvState(); persistHex(rgbToHex(...Object.values(hsvToRgb(hsv.h,hsv.s,hsv.v)))); }
  ...
  if(colorResetBtn) colorResetBtn.addEventListener('click', (e)=>{ e.stopPropagation(); try{ localStorage.setItem(STORAGE_KEY, DEFAULT_BG_HEX); }catch(err){} const {r,g,b}=hexToRgb(DEFAULT_BG_HEX); const hv=rgbToHsv(r,g,b); hsv.h = hv.h; hsv.s=hv.s; hsv.v=hv.v; applyHsvState(); updatePickerUI(); hideColorPopup(); });
  ...
  function isVisualBlurEnabled(){
    try { const v = localStorage.getItem(BLUR_VISUAL_KEY); if(v===null) return true; return v === 'true'; } catch(e){ return true; }
  }
  ...
  function captureGlowInfo(){
    ...
    glowEls.forEach(el => {
      try {
        ...
        const rgb = (colStr && colStr.match) ? (colStr.match(/rgba?\(\s*([0-9]+)[,\s]+([0-9]+)[,\s]+([0-9]+)/i) || null) : null;
        if(rgb) el.style.setProperty('--glow-rgb', `${rgb[1]}, ${rgb[2]}, ${rgb[3]}`);
  ...
  function initBlurTargetsForChapter(filename, blurEnabled = true){
    if(!chapterBodyEl) return;
    captureGlowInfo();
    chapterBodyEl.querySelectorAll('.blur-target').forEach(old => { old.classList.remove('is-blurred','hover-reveal'); old.classList.remove('blur-target'); });
  ...
  window.addEventListener('resize', ()=>{ checkAndUnblurVisibleTargets(); scheduleEdgePosUpdate(); scheduleGlitchUpdate(); });
  ...
  /* ---------- tippy init for .gloss with image support ---------- */
  ...
  window.addEventListener('resize', ()=>{ positionTopNav(); onScrollCheck(); scheduleGlitchUpdate(); });
  ...
  if(!document.getElementById('image-overlay')){
    const overlay = document.createElement('div'); overlay.id = 'image-overlay';
  ...
  /* ---------- persist scroll on page reload only ---------- */
  ...
  /* ---------- GLITCH functions (new) ---------- */
  function initGlitchElements(){
    _glitchEls = [];
    if(!chapterBodyEl) return;
    const els = Array.from(chapterBodyEl.querySelectorAll('.glitch'));
    els.forEach(el => {
      const base = parseFloat(el.getAttribute('data-glitch-intensity'));
      const baseVal = Number.isFinite(base) ? base : 0.5;
      el.style.setProperty('--glitch-base', String(baseVal));
      el.style.setProperty('--glitch-scale', '1');
      const txt = (el.textContent || '').replace(/^\n+|\n+$/g,'');
      el.setAttribute('data-glitch', txt);
      _glitchEls.push({ el, base: baseVal });
    });
    updateGlitchValues();
  }

  function updateGlitchValues(){
    if(!_glitchEls.length) return;
    const vh = window.innerHeight || 800;
    const centerY = vh / 2;
    _glitchEls.forEach(item => {
      const el = item.el;
      try {
        const rect = el.getBoundingClientRect();
        ...
        const multiplierFactor = 4.0;
        const scale = 1 + (norm * multiplierFactor);
        el.style.setProperty('--glitch-scale', String(scale));
        const combined = Math.max(0, item.base * scale);
        el.style.setProperty('--glitch-intensity-combined', String(combined));
        const targetOpacity = Math.min(1, combined * 0.22);
        el.style.setProperty('--glitch-opacity', String(targetOpacity));
      } catch(e){}
    });
  }

  function scheduleGlitchUpdate(){
    if(_glitchScheduled) return;
    _glitchScheduled = true;
    requestAnimationFrame(()=>{ updateGlitchValues(); _glitchScheduled = false; });
  }

  window.addEventListener('scroll', scheduleGlitchUpdate, { passive:true });
  window.addEventListener('resize', scheduleGlitchUpdate);
  ...
  async function loadChapter(filename, title){
    ...
    try {
      ...
      // initialize new features on content
      initGlitchElements();
      ...
      updateEdgeScrollVisibility(); updateEdgeScrollPosition();
    } catch(err){
  ...
  if(blurToggle){
    ...
    blurToggle.addEventListener('click', (e)=>{ e.stopPropagation(); const newState = !isVisualBlurEnabled(); setVisualBlurEnabled(newState); });
  }

  /* ---------- EDGE SCROLL BUTTON (fixed & flush) ---------- */
  function createEdgeScrollButton(){
    ...
    edgeBtn.addEventListener('click', (e)=>{
      ...
      if(!next) return;
      // remove visual blur to allow smooth transition (CSS handles transition)
      next.classList.remove('is-blurred');
      // do NOT add .unblurred here to preserve read semantics; actual marking happens when user scrolls to it
      // scroll to center
