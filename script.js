// script.js - full replacement (viewer sizing improved + clickable cursors)
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function resolveTooltipImage(srcCandidate){
    ...
    for(const u of unique){
      ...
      if(await testImageUrl(u)){ console.debug('tippy-img: resolved', srcCandidate, '->', u); resolvedUrlCache.set(srcCandidate, u); return u; }
    }
  ...
  /* preload tooltip images for the chapter */
  ...
  function initGlossTippy(){
    ...
    tippy('.gloss', {
      ...
      appendTo: () => document.body, popperOptions: { strategy: 'fixed' }, content: 'Loading...',
      onShow: async (instance) => {
        ...
        if(resolved){
          ...
          if(cachedImg && cachedImg.complete){
            ...
            imgClone.addEventListener('click', (ev) => { ev.stopPropagation(); try{ openImageViewer(resolved, imgAlt); }catch(e){} try{ instance.hide(); }catch(e){} });
            ...
            img.alt = imgAlt;
            img.loading = 'eager';
            img.addEventListener('load', () => { try{ if(instance.popperInstance && typeof instance.popperInstance.update==='function') instance.popperInstance.update(); else if(typeof instance.update==='function') instance.update(); }catch(e){} });
            img.addEventListener('click', (ev) => { ev.stopPropagation(); try{ openImageViewer(resolved, imgAlt); }catch(e){} try{ instance.hide(); }catch(e){} });
  ...
  let lastScrollY = window.scrollY; let scheduled = false; let hideDelayTimer = null; const HIDE_DELAY_MS = 2500;
  ...
  function openImageViewer(src, alt = ''){
    // set src and defensive sizing to ensure full image fits inside viewport
    overlayImg.src = src;
    overlayImg.alt = alt || '';
    // Defensive CSS via JS (same values as CSS max sizes)
    const margin = 40; // keep in sync with --viewer-margin
    overlayImg.style.maxWidth = `calc(100vw - ${margin}px)`;
    overlayImg.style.maxHeight = `calc(100vh - ${margin * 1.5}px)`;
    ...
    // center scroll to top-left so users see whole image
  ...
  /* preload & tippy code unchanged from previous version (keeps behavior) */
  async function preloadTooltipImages(){
    if(!chapterBodyEl) return;
    const glossEls = Array.from(chapterBodyEl.querySelectorAll('.gloss'));
    if(!glossEls.length) return;
    for(const el of glossEls){
      ...
      }catch(err){ console.debug('tippy-img: preload error for', dataImg, err); }
    }
  }

  // resolveTooltipImage, testImageUrl copied (unchanged)
  function testImageUrl(url, timeout = 3000){
    return new Promise((resolve) => {
      const img = new Image();
      let done = false;
  ...
  /* nav tippies */
  function refreshNavTippies(){ if(!window.tippy) return;
    [bottomPrev, bottomNext, topPrev, topNext].forEach(btn => { if(!btn) return; try{ if(btn._tippy) btn._tippy.destroy(); }catch(e){} });
    if(bottomPrev) tippy(bottomPrev, { content: () => bottomPrev.dataset.title || '', placement: 'top', delay: [80,40], offset: [0,8], appendTo: ()=>document.body });
    if(bottomNext) tippy(bottomNext, { content: () => bottomNext.dataset.title || '', placement: 'top', delay: [80,40], offset: [0,8], appendTo: ()=>document.body });
    if(topPrev) tippy(topPrev, { content: () => topPrev.dataset.title || '', placement: 'bottom', delay: [80,40], offset: [0,8], appendTo: ()=>document.body });
  ...
  let lastScrollY2 = window.scrollY; let scheduled2 = false;
  function onScrollCheck2(){
    const curY = window.scrollY; const scrollingUp = curY < lastScrollY2; const atTop = curY <= 10;
    if(bottomNavIsVisible()){ hideTopNav(); if(hideDelayTimer){ clearTimeout(hideDelayTimer); hideDelayTimer=null; } }
    else if(atTop || scrollingUp){ if(hideDelayTimer){ clearTimeout(hideDelayTimer); hideDelayTimer=null; } showTopNav(); }
    else { scheduleHideTopNavWithDelay(); }
    lastScrollY2 = curY;
  }
  window.addEventListener('scroll', ()=>{ if(scheduled2) return; scheduled2 = true; requestAnimationFrame(()=>{ onScrollCheck2(); scheduled2=false; }); }, { passive:true });
  window.addEventListener('resize', ()=>{ positionTopNav(); onScrollCheck2(); });
